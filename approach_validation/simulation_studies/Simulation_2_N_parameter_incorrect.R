...
# This simulation tests the role of misspecifying the input parameter N upon penetrance estimation accuracy
  ...
  for(p in 1:ncol(sibships)){
    ...
    for(s in 1:length(states)){
      ...
      for(n in 1:length(modif)){
        ...
        for(t in 1:2){
          suppressMessages(
            adpen_estimate <- adpenetrance(N=obsN+modif[n],
...
Sim2 <- ddply(sum_diff,.(Population,Tailoring,TruePenetrance,States.Modelled,modif),summarise,CorrMinusTrue=mean(CorrMinusTrue),UncorrMinusTrue=mean(UncorrMinusTrue))

#Convert states modelled column into factors with appropriate names
Sim2$States.Modelled <- factor(Sim2$States.Modelled,levels= c(1:5),labels= c("F, S, U",
                                                                             "F, S",
                                                                             "F, U",
                                                                             "S, U",
                                                                             "A, U"))

Sim2$Population <- factor(Sim2$Population,levels= c(1:2),labels= c("UK (1974)",
                                                                   "Next Steps"))

Sim2$Tailoring <- factor(Sim2$Tailoring,levels= c(1:3),labels= sim_labs <- c("Poisson",
                                                                             "Tailored",
                                                                             "No correction"))


#Melt so that the corrected and uncorrected penentrance estiamtes are in the same column
mSim2<- reshape2::melt(Sim2,measure.vars=c("CorrMinusTrue","UncorrMinusTrue"))
...
mSim2 <- mSim2[-which(mSim2$variable=="UncorrMinusTrue" & mSim2$Tailoring=="Tailored"),] 

#Then relabel uncorrected into a third 'tailoring' category
mSim2$Tailoring[which(mSim2$variable=="UncorrMinusTrue")] <- "No correction"

fig <- ggplot(data=mSim2, aes(modif,value,colour=factor(TruePenetrance)))+
  geom_line()+
...
ggsave(filename="Simulation_2.pdf",plot=fig,device='pdf',units="mm",width=210,height=210,dpi=300)
write.table(mSim2,file="Simulation_2.tsv",sep="\t",quote=FALSE,row.names = FALSE,col.names = TRUE) 
