...
#It is used for validation of the adpenetrance approach to penetrance estimation using in simulated datasets
  #This simulation generates two example populations (defined based on real sample populations)
    ...
    #when the RX input parameter is not correctly defined, with various degrees of error
...
#### Population has been defined - now estimate penetrance and produce summary
    ...
    for(s in 1:length(states)){
      ...
      for(n in 1:length(modif)){
        ...
        #Run Adpenetrance for the states modelled at the given f value
          ...
          #RX is defined by the modified value derived above
        adpen_estimate <- adpenetrance(N=obsN,RX=modifiedRX,states=states[s])
        ...
        #Run Adpenetrance for the states modelled at the given f value
          ...
          #RX is defined by the modified value derived above
        adpen_estimateTailored <- adpenetrance(N=obsN,RX=modifiedRX,states=states[s],define_sibstructure = sibstructure)
  ...
  ##write.csv(Sim3,file=paste0("Simulation3_facet_grid_",Sys.Date(),".csv"))
  
  #Convert states modelled column into factors
  Sim3$States.Modelled <- as.factor(Sim3$States.Modelled)
  ...
  ggplot(data=Sim3 ,aes(modif,CorrMinusTrue,colour=factor(TruePenetrance)))+
    geom_line()+
    ...
    ggsave(path="Supp_figures",filename=paste0("Figure_S6.png"),units="mm",width=210,height=210,dpi=300)
    #ggsave(path="Supp_figures",device='pdf',filename=paste0("Figure_S6.pdf"),units="mm",width=210,height=210,dpi=300)
